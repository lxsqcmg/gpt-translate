...
contextual_links:
  ...
  - type: subtitle
    name: "博客文章"
  - type: link
    name: "介绍域捕获：将您的组织的 Postman 用户分组为一个团队"
    url: "https://blog.postman.com/introducing-domain-capture/"

---

> **[域验证和捕获仅适用于 Postman 企业版计划。](https://www.postman.com/pricing)**

验证域以查看在使用您的组织的域名和子域名创建的所有 Postman 用户帐户。启用域捕获以管理所有这些 Postman 用户帐户。使用域捕获，您可以将您组织的所有 Postman 用户合并为一个 Postman 团队，并确保使用您的域名注册 Postman 的任何新用户都会自动添加。

要启用域捕获，[团队管理员](/docs/collaborating-in-postman/roles-and-permissions/#team-roles)必须首先[添加和验证域名或子域名](#adding-and-verifying-a-domain)。一旦域名已验证，他们可以创建一个请求来[启用域捕获](#enabling-domain-capture)。一旦请求获得批准，他们可以[联系 Postman 支持](#contacting-support-to-manage-accounts)来管理[未认领的帐户](#viewing-unclaimed-accounts)。

## 目录

* [域验证和捕获的先决条件](#prerequisites-for-domain-verification-and-capture)
...
* [添加和验证域名](#adding-and-verifying-a-domain)
    ...
    * [查看未认领的帐户](#viewing-unclaimed-accounts)

* [启用域捕获](#enabling-domain-capture)
...
* [域捕获常见问题解答](#domain-capture-faqs)

## 域验证和捕获的先决条件

您必须是[Postman 团队管理员](/docs/collaborating-in-postman/roles-and-permissions/#team-roles)，才能为您的团队添加和验证域名，并为您的团队启用域捕获。

以下是添加和验证域名并启用域捕获所需的条件：
...
建议您启用[SSO 配置](/docs/administration/scim-provisioning/scim-provisioning-overview/)以简化新用户的入职流程，并启用[Auto-Flex](/docs/administration/billing/#using-auto-flex)以确保您的团队可以自动适应可能添加到您的 Postman 团队中的所有用户，当启用域捕获时。

## 添加和验证域名

[团队管理员](/docs/collaborating-in-postman/roles-and-permissions/#team-roles)可以通过导航到[域捕获仪表板](https://go.postman.co/settings/team/domain-capture)向您的团队添加域名和子域名。一旦您将验证**TXT 记录**添加到您的 DNS 配置中，Postman 团队将验证您的域名或子域名。在您可以查看和管理使用您的组织的域名和子域名在 Postman 中创建的所有用户帐户之前，必须验证您的域名。

1. 打开 Postman 并在 Postman 标头中选择 **Team > Team Settings**。在左侧边栏中选择 **Authentication**。
1. 确认您的团队仅启用了 SSO 身份验证方法。然后，选择 **Domains** 选项卡。

    > 您的团队必须仅启用基于 SAML 的 SSO 身份验证方法才能设置域捕获。必须关闭替代身份验证方法，例如 Postman 或 Google 登录。
...
1. 选择 <img alt="复制图标" src="https://assets.postman.com/postman-docs/icon-copy-v9.jpg#icon" width="15px"> **Copy** 以复制 **TXT 记录**，然后将其添加到您的域名的 DNS 配置中。
    ...
    > 如果您还没有准备好将 **TXT 记录** 添加到您的域名的 DNS 配置中，则可以选择 **Verify Later**。要稍后验证您的域名，请参阅[稍后验证域名](#verifying-a-domain-later)。

1. 选择 **I have added the TXT record to the DNS tool** 复选框，然后选择 **Verify Domain**。请注意，可能需要长达 24 小时才能使 TXT 更改生效，以启用域的成功验证。当域名验证成功时，团队管理员将收到电子邮件通知。

    <img alt="添加域 TXT 记录" src="https://assets.postman.com/postman-docs/v10/add-domain-verification-record-v10.jpg" width="450px"/>

    > 您可以联系[Postman 支持](https://www.postman.com/support/)，以检查您的域验证请求的状态。

1. 一旦您的域名已验证，您可以[创建请求以启用域捕获](#enabling-domain-capture)。
...
1. 打开 Postman 并在 Postman 标头中选择 **Team > Team Settings**。在左侧边栏中选择 **Authentication**。
1. 选择要验证的状态为 `UNVERIFIED` 的域名旁边的 **Verify**。

    > 您还可以选择要验证的状态为 `UNVERIFIED` 的域名旁边的 **Manage**，然后选择 **View Verification Details**。

    ![稍后验证域名](https://assets.postman.com/postman-docs/v10/verify-domain-later-v10.jpg)

1. 选择 <img alt="复制图标" src="https://assets.postman.com/postman-docs/icon-copy-v9.jpg#icon" width="15px"> **Copy** 以复制 **TXT 记录**，然后将其添加到您的域名的 DNS 配置中。
1. 选择 **I have added the TXT record to the DNS tool** 复选框，然后选择 **Verify Domain**。Postman 团队将检查您请求的域名，并在您的域捕获仪表板中将其状态更新为 `VERIFIED`。当域名验证成功时，团队管理员将收到电子邮件通知。
...
1. 打开Postman并在Postman标题中选择**团队>团队设置**。在左侧边栏中选择**身份验证**。
2. 选择状态为`已验证`的域旁边的**管理**，然后选择**查看帐户**。

    > 可选地，您可以在域名仪表板中选择域名旁边的未认领帐户数量。
...
1. 打开Postman并在Postman标题中选择**团队>团队设置**。在左侧边栏中选择**身份验证**。
2. 选择状态为`已验证`的域旁边的**管理**，然后选择**请求认领**。然后，Postman支持将联系团队管理员以解释启用域名捕获的影响。
3. 一旦请求获得批准，您可以[联系Postman支持](#联系支持以管理帐户)来管理[未认领的帐户](#查看未认领的帐户)。
...
> 您将无法有选择地选择您想要添加到团队的用户。一旦启用，未认领帐户的数量将减少，因为这些用户登录并加入您的团队。

之前在其他Postman团队中的现有用户将失去其早期团队的所有数据，包括个人工作区。不会将任何数据从他们的早期团队转移到他们的新团队。用户将无法留在或加入其他Postman团队，使用他们的捕获帐户。
...
一旦获得批准请求[启用域名捕获](#启用域名捕获)，团队管理员可以通过导航到[域名仪表板](https://go.postman.co/settings/team/domain-capture)来联系Postman支持。联系Postman支持以认领不属于您团队的帐户，将团队合并到您的团队中，并限制用户仅在您的团队中工作。

1. 打开Postman并在Postman标题中选择**团队>团队设置**。在左侧边栏中选择**身份验证**。
2. 选择状态为`已验证`的域旁边的**管理**，然后选择**联系支持**。
...
* 如果被捕获的用户不属于任何团队：
    ...
    * 当用户使用SSO身份验证进入您的组织团队时，他们的现有数据将被转移到该团队。
* 如果被捕获的用户是现有免费或付费团队的一部分：
    * 他们将从现有团队中删除。
    * 所有现有用户会话将被撤销。
    * 用户下次登录时，将提示使用SSO加入您的组织团队。
    * 用户可以使用SSO身份验证进入您的组织团队。
    * 他们将失去来自早期团队的所有数据，包括个人工作区。不会将任何数据从早期团队转移到新团队。
...
* 如果被捕获的用户是现有付费团队的最后一个管理员：
    * 在验证您添加到团队的域之前，Postman支持将与您的团队管理员联系以讨论如何处理此问题。
    * 所有现有用户会话将被撤销。
...
### 团队管理员可以在启用域捕获之前查看将被捕获的用户帐户列表吗？

是的，团队管理员可以在确认要启用域捕获之前查看将被捕获的未认领的Postman帐户列表。有关更多信息，请参见[查看未认领的帐户](#viewing-unclaimed-accounts)。

### 添加域是否会自动将组织域中的所有团队成员添加到组织团队中？

不会，团队管理员添加域后，必须创建请求，请求Postman团队验证域或子域。一旦满足这两个标准，团队管理员可以创建请求以启用域捕获。然后，Postman支持将联系团队管理员，以解释启用域捕获的影响。有关更多信息，请参见[启用域捕获](#enabling-domain-capture)。